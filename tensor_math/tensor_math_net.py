...
class Network():
    ...
    def initialize_random_weights_biases(self):
        ...
        self.biases = [tf.random.uniform((y, 1), -1, 1, dtype=tf.float32) for y in self.size[1:]]
        self.weights = [tf.random.uniform((y, x), -1, 1, dtype=tf.float32) for x, y in zip(self.size[:-1], self.size[1:])]
    ...
    def backprop(self, activation: tf.Tensor, y: np.ndarray):
  ...
  the net\n
        ...
        if self.weights and self.biases:
            ...
            nabla_w = [tf.zeros(w.shape) for w in self.weights]
            nabla_b = [tf.zeros(b.shape) for b in self.biases]
            ...
            for l in range(2, self.num_of_layers):
                z = Z_vectors[-l]
                delta = tf.tensordot(tf.transpose(self.weights[-l+1]), delta, 1) * sigmoid_prime(z)
                nabla_b[-l] = delta
